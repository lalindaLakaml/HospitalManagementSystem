...
<div class="container mt-4">
    ...
    <h6 class="mt-5"><b>Search Bed</b></h6>
    ...
    <table class="table table-striped">
        ...
        <tbody id="showdata">
          <?php  
                $sql = "SELECT bed.Bed_ID,war.Description AS ward, typ.Description AS type  FROM `bed` AS bed , wards AS war, bed_types AS typ  WHERE bed.Wor_ID = war.Wor_ID  AND bed.Bed_Typ_ID = typ.Bed_Typ_ID";
                $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query))
                {
                  echo"<tr>";
                   echo"<td><h6><a href=# onclick=myClose(".$row['Bed_ID'].");>".$row['Bed_ID']." </a></h6></td>";
                   echo"<td><h6>".$row['type']."</h6></td>";
                   echo"<td>".$row['ward']."</td>";
                  echo"</tr>";   
                }
