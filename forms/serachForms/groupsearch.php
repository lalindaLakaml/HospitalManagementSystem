...
<div class="container mt-4">
    ...
    <table class="table table-striped">
        ...
        <tbody id="showdata">
          <?php  
                $sql = "SELECT `Gro_ID`,`Description`,`Memo` FROM `system_user_groups` WHERE `isActive` =1";
                $query = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_assoc($query))
                {
                  echo"<tr>";
                   echo"<td><h6><a href=# onclick=myClose(".$row['Gro_ID'].");>".$row['Gro_ID']." </a></h6></td>";
